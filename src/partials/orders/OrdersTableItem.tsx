...
import {OrdersTableItemProps} from '../../utils/types';
import DefStatus from '../../assets/default-status.svg';
import IconError from '../../assets/error.svg';

function OrdersTableItem(props: OrdersTableItemProps) {
  ...
  const iconError = (error: string) => {
    if(error === ''){
        ...
        return <img className="ml-1 inline-block" src={IconError} alt="" />
    }
  };
      ...
      <tr className="hover:bg-[#F5F5F5] transition-all">
        ...
        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
          <div className="flex items-center text-slate-800">
            <div className="w-10 h-10 shrink-0 flex items-center justify-center mr-2 sm:mr-3">
              <img className="ml-1" src={props.image} width="26" height="26" alt={props.order} />
            </div>
            <div className="font-medium text-sky-500">
                <Link className="href-primary " to="/order">
                    {props.order} {iconError(props.error)}
                </Link>
                <div className="text-[10px] text-description">{props.orderdesc}</div>
            </div>
          </div>
        </td>
        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        ...
        <td className="px-2 first:pl-5 last:pr-5 py-3 min-w-[300px]">
          <div className={`inline-flex font-medium rounded-[5px] text-left px-2.5 py-0.5 bg-[#F4F5F7]`}>{/*${statusColor(props.status)}*/}
            {typeIcon(props.type)}
            {props.status}
          </div>
        </td>
